function [G, CG, basis, pv_out] = run_compareParams_2phFiveSpot(K_frac, paramstr,  paramval, paramcount)
    output = true;
    if (size(paramval, 1) > 2)
        output = false;
    end
    ...
    basis_sb = getMultiscaleBasis(CG, A, 'type', 'rsb', paramstr, paramval(paramcount));
    if (output)
